...
Public Class PublicFunction
    ...
    Public Shared Sub SendSMS(ByVal Param As String())
        Try
            PublicFunction.CheckGSMModem()
            If Setting.Signal > 0 And Param(1) <> "" Then
                Setting.GSMCom.Open()
                Dim pdus As OutgoingSmsPdu() = Nothing
                pdus = SmartMessageFactory.CreateConcatTextMessage(Param(0), True, Param(1))
                If (Not pdus Is Nothing) Then
                    Dim num As Integer = pdus.Length
                    Dim i As Integer = 0
                    For Each pdu As OutgoingSmsPdu In pdus
                        i += 1
                        Setting.GSMCom.SendMessage(pdu)
                    Next
                End If
                Thread.Sleep(1000)
                If Setting.GSMCom.IsOpen Then
                    Setting.GSMCom.Close()
                End If
                Dim dr As DataRow = BaseDataObject.DataAccess.NewRow("Pm_SMS")
                dr("Matn") = Param(0)
                dr("Tarikh") = Setting.TarikheJary
                dr("Time") = Setting.Now.Hour.ToString() + ":" + Setting.Now.Minute.ToString() + ":" + Setting.Now.Second.ToString()
                dr("Srl_Ashkhas_Reciever") = Param(3)
                dr("Srl_Ashkhas_Sender") = Setting.Srl_User
                dr("Mobile") = Param(1)
                dr("Srl_Pm_EibFormMaster") = Param(2)
        ...
        End Try
    End Sub
    Private Sub SendMultiple(ByVal pdus As OutgoingSmsPdu())
        Dim num As Integer = pdus.Length
        Try
        ...
        End Try
    End Sub
    Public Shared Sub CheckGSMModem()
        Dim FindModem As Boolean = False
        For Each st As String In SerialPort.GetPortNames()
            Setting.GSMCom = New GsmCommMain(st, 115200, 300)

            Try
                If Not Setting.GSMCom.IsOpen() Then
                    Setting.GSMCom.Open()
                End If
            Catch ex As Exception
                Continue For
            End Try
            FindModem = Setting.GSMCom.IsConnected()
            If FindModem Then
                Try
                    Dim info As SignalQualityInfo = Setting.GSMCom.GetSignalQuality()
                    Setting.Signal = info.SignalStrength
                    Setting.PortName = st
                    If Setting.GSMCom.IsOpen() Then
                        Setting.GSMCom.Close()
                    End If
  ...
  "      dbo.Sec_Setting.ObjectType, dbo.Sec_Setting.PropName, dbo.Sec_Setting.PropValue, dbo.Sec_Setting.PropImage,dbo.Sec_Setting.Dis, dbo.Sec_Setting.Visible, dbo.Sec_Setting.Active " & _
