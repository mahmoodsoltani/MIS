Imports System.IO
Public Class Frm_RpStatusStatementExtraWork
    ...
    Public Function RemoveZero(ByVal Inputdt As DataTable) As DataTable
        For i As Integer = 0 To Inputdt.Rows.Count - 1
            If Not i < Inputdt.Rows.Count Then
                Return Inputdt
            End If
            If (Inputdt.Rows(i)(11).ToString() = "" Or Inputdt.Rows(i)(11).ToString() = "0:0" Or Inputdt.Rows(i)(11).ToString() = "00:00" Or Inputdt.Rows(i)(11).ToString() = "00:00:00") And (Inputdt.Rows(i)(12).ToString() = "" Or Inputdt.Rows(i)(12).ToString() = "0:0" Or Inputdt.Rows(i)(12).ToString() = "00:00" Or Inputdt.Rows(i)(12).ToString() = "00:00:00") Then
    ...
    Private Sub btn_Save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Save.Click
        ...
        Else
            rp = New Rp_MultiStatusExtraWorkmashrih
        End If
        Dim dt As DataTable = BaseDataObject.DataAccess.GetDataTable("exec [Rp_MultiEzafeKarstatusstatement] '" + Person + "','" + txt_FromDate.Value + "','" + txt_ToDate.Value + "'," + Company.ToString() + "," + Srl_Vahed.ToString())
        If rb_Accounting.Checked Then
            dt = MergDate(dt)
        Else
            dt = RemoveZero(dt)
        End If
        If rb_Accounting.Checked Then
            Dim dt1 As DataTable = BaseDataObject.DataAccess.GetDataTable("exec [Rp_StatusHazineCodeEzafeKar_Modified] '" + Person + "','" + txt_FromDate.Value + "','" + txt_ToDate.Value + "'," + Company.ToString() + "," + Srl_Vahed.ToString()).Copy()
            ' RemoveZeroHazine(dt1)
            rp.Subreports(0).SetDataSource(dt1)
            Dim Confirmdt As DataTable = BaseDataObject.DataAccess.GetDataTable("exec [Rp_ConfirmedMultiEzafeKarStatusStatement_BasedOnAll] '" + Person + "','" + txt_FromDate.Value + "','" + txt_ToDate.Value + "'," + Company.ToString() + "," + Srl_Vahed.ToString())
            rp.Subreports(1).SetDataSource(Confirmdt)

        Else
            Dim Confirmdt As DataTable = BaseDataObject.DataAccess.GetDataTable("exec [Rp_ConfirmedMultiEzafeKarStatusStatement_BasedOnAll] '" + Person + "','" + txt_FromDate.Value + "','" + txt_ToDate.Value + "'," + Company.ToString() + "," + Srl_Vahed.ToString())
            rp.Subreports(0).SetDataSource(Confirmdt)
    ...
    Private Sub btn_File_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_File.Click
        ...
        If FolderBrowserDialog1.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
            ...
            For Each dr As DataRow In dt1.Rows
                temp = dr(1).ToString() + "," + dr(2).ToString() + "," + dr(4).ToString() + "," + dr(3).ToString() + "," + dr(5).ToString()
                sw.WriteLine(temp)
            Next
            sw.Close()
    ...
    Private Sub Cmb_Company_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmb_Company.SelectedIndexChanged, cmb_vahed.SelectedIndexChanged
        Me.Pm_AshkhasWithCompanyTableAdapter.Fill(Me.Ds_Ashkhas1.Pm_AshkhasWithCompany, Cmb_Company.SelectedValue)
    End Sub

    Private Sub CheckBox1_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk_AllVahed.CheckedChanged
